...
扫码成功后手机会记住这台电脑及其候选 IP 池。下次启动时，手机会直接尝试保存过的地址候选；即使公司、学校或宿舍网络隔离广播，只要设备间单播可达，也能自动连回。
...
1. 桌面端展示包含 `device_id/ip/ips/port/name/os` 的配对二维码
...
5. 仍然不行就点手机端"刷新连接"，或重新扫描 PC 端二维码更新候选 IP 池
