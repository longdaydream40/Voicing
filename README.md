...
和 AI 对话时打字太慢？Voicing 让你用嘴代替手——手机语音输入，直接打到电脑上
...
Voicing 把手机的语音输入法变成电脑的"嘴"——你在手机上说话，文字直接出现在电脑光标处。用语音快速调度 Claude Code、Codex、Cursor 等 AI Agent，效率远超打字

- **双传输模式** — Windows / Linux 走蓝牙，macOS 走局域网 WiFi
...
| 平台 | 文件 | 要求 |
|------|------|------|
| Windows | `voicing-windows-x64.exe` | Windows 10/11 (64-bit) + 蓝牙 |
| macOS | `voicing-macos-arm64.dmg` | macOS 15+ Apple Silicon |
| Linux | `voicing-linux-x86_64` | Ubuntu 24.04+ GNOME X11 + 蓝牙 |
| Android | `voicing.apk` | Android 5.0+ |

### 2. 连接

```
Windows / Linux：系统蓝牙完成配对 → 运行桌面端程序 → Android 选择蓝牙
macOS：电脑和手机连同一局域网 → 运行桌面端程序 → Android 选择 WiFi
```

- **Windows**：运行 `voicing-windows-x64.exe`，先在系统蓝牙设置中与手机完成配对
- **macOS**：打开 `voicing-macos-arm64.dmg`，拖入 Applications（首次打开需右键→打开），电脑和手机连接同一 WiFi 路由器
- **Linux**：`chmod +x voicing-linux-x86_64` 后运行，先在系统蓝牙设置中与手机完成配对
...
1. **Windows / Linux**：桌面端通过经典蓝牙 RFCOMM 暴露服务，Android 连接已配对设备
2. **macOS**：桌面端继续通过 UDP 广播 + WebSocket 在局域网内提供服务
...
├── pc/                          # 桌面端 (Python + PyQt5)
│   ├── voice_coding.py          # 主程序
│   ├── bluetooth_server.py      # Windows/Linux RFCOMM 服务
...
注意：
- 本地 Android release 构建需要兼容的 JDK 17/21（当前 GitHub Actions 发布环境固定使用 Java 17）
- Windows / Linux 蓝牙模式需要先在系统蓝牙设置中与手机完成配对
- macOS 首次打开可能提示"无法验证开发者"，右键→打开即可
- Linux 需要先 `chmod +x` 赋予执行权限
- Linux 仅支持 X11 会话，Wayland 下文本注入不可用（程序会自动检测并提示）

## 常见问题

**手机无法连接电脑？**
1. 如果是 Windows / Linux，先确认手机和电脑已经在系统蓝牙设置中完成配对
2. 如果是 macOS，确认电脑和手机在同一个 WiFi 路由器下
3. macOS 模式检查电脑防火墙是否放行 UDP 9530 端口
