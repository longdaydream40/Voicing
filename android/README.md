...
- 启动、恢复前台和手动刷新时直接按保存过的 IP 候选重连，支持热点和公司/学校局域网来回切换
...
2. 电脑端托盘菜单点击"显示 QR 码"
3. 手机连接同一网络，打开 Voicing
4. 首次使用或换电脑后，点击手机端"更多功能操作"里的"扫码连接"，扫描 PC 端二维码保存这台电脑
5. 状态栏显示"已连接"即可使用；后续启动会直接尝试保存过的 IP 候选。若电脑和手机一起切换到新局域网，等待 PC 端刷新 QR 码后重新扫码即可把新 IP 合并进候选池，无需重启桌面端；再切回旧局域网时会继续尝试旧候选 IP
6. 切换到语音输入法，说话，文字自动出现在电脑光标处
7. 点击"更多功能操作"可开启"自动 Enter"（适合聊天场景，语音分段输入只会补一次 Enter）
